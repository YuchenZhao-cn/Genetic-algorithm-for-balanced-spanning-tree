package BalancedSpanningTree;

import java.util.*;

/**

* Genetic Algorithm for Balanced Spanning Tree

* @author Yuchen Zhao

* @Time 2020-12-04 12:47:01

*/
public class BalancedSPT_GA {
	private Edge[] Tm; // Minimum Spanning Tree
	private int[] Ts; // Shortest Path Tree
	private population p1 = new population(); // population of the individuals
	private population p2 = new population(); // auxiliary population
	private double α; // α, which means no distance in the spanning tree is more than α * minimum distance
	private double β; // β, which equals total weight of the best individual divided by the weight of the minimum spanning tree
	private Edge[] graph; // The input graph
	private int source; // The source of the graph
	private int V, E, pop, minWeight; // V - number of vertices, E - number of edges, pop - number of individuals, minWeight - total weight of Tm
	private double pc, pm; // pc - crossover possibility, pm - possibility of mutation
	private int w1 = -1, w2 = -1, w3 = -1; // auxiliary variables for the termination point
	...
	* @Time 2020-12-04 13:11:34

	*/
	private class population {
		private int pop = 0; // number of individuals in the population
		private ArrayList<RandomSPT> individuals = new ArrayList<RandomSPT>(); // List of individuals
		...
		// prioirty is descided by the weight of the tree
		private void kill(int pop) {
			...
			while(individuals.size() > pop) {
				...
				// The smaller the weight is, the less possibility it is to be killed
	...
	/**
	 * Constructor
	 * @param graph, pop, maxgen, pc, pm, source, α
	 * @Description Executes genetic algorithms including initialize, crossover, mutation
	 */
	public BalancedSPT_GA(int[][] graph, int pop, int maxgen, double pc, double pm, int source, double α) {
		this.α = α;
		this.source = source;
		this.pop = pop;
		this.pc = pc;
...
//			if(w1 == w2 && w2 == w3) break;
		...
		// Output
		β = (double)p1.best().weight / (double)minWeight;
		System.out.print("------------\nBest individual: \n" + p1.best());
		System.out.println("β = " + β);
		System.out.println("Generations: " + gen);
		System.out.println("source = " + (char)(source + 65) + ",  α = " + α + ",  pop = " + pop + ",  maxgen = " + maxgen + ",  pc = " + pc + ",  pm = " + pm);
	}
	...
	*/
	...
	/**
	* Judge whether the tree meets the requirement of α
	* @param edges
	* @return true when the input tree meets the requirement of α
	*/
	...
	* @param edges
	* @return The sequence number of the edge that is selected
	*/
	...
	* @param edges, e
	* @return The tree that has been fixed the cycling problem
	*/
	private Edge[] deleteCycle(Edge[] edges, Edge e) {
		...
		// Find the cycle
		// Repeatedly deleting edges that holds vertices only appears one time
		for(int i = 0; i < edges.length; i++) {
		...
		// Fixing the cycle
		// add e to the edges and deleting one edge randomly from the cycle
		for(int i = 0; i < edges.length; i++) {
			if(edges[i] != null) num++;
		}
	...
	* Crossover
	* @param par1, par2
	* @Description Add the offsprings from the crossover of par1 and par2 to the population
	*/
	private void crossover(Edge[] par1, Edge[] par2) {
		...
		// Find out the edges that appears in both spanning trees
		for(int i = 0; i < V - 1; i++) {
			for(int j = 0; j < V - 1; j++) {
		...
		// Find out the edges that are unique in both trees
		...
		// add unique edges into parents and fix the cycling problems
		T1 = parent1;
		T2 = parent2;
	...
	* Mutation
	* @param T
	* @Description Add the offspring from the mutation of T to the population
	*/
	private void mutation(RandomSPT T) {
		...
		// Randomly delete edges in the tree and add edges into the tree
		// Add trees that don't have cycles and meet the requirement of α
	...
	/**
	* return the remaining edges in the graph graph excepts the tree
	* @param t
	* @return the remaining edges in the graph graph excepts the tree t
	*/
	...
	public static void main(String[] args) {
		...
		System.out.println("They are written on the line 162 and 163 and are ready to be used.");
	}
